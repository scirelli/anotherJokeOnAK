...
int WINAPI WinMain(	HINSTANCE hinstance, HINSTANCE hprevinstance, LPSTR lpcmdline, int ncmdshow)
{
	...
	while(1)
	{
    	...
    	hmdc= GetDC( GetTopWindow(hwnd) ); //gets the top window of the this window
												 //seems to also work with GetDC but this draws on
												 //top of any window below it (visually)
		
		SelectObject(hmdc, GetStockObject(BLACK_BRUSH));
		SelectObject(hmdc, black_pen);
		Rectangle(hmdc, 0, 0, 1240, 1240);
		
		SetBkMode(hmdc, TRANSPARENT); //set background color
		SetTextColor(hmdc,RGB(255,255,255)); //set text color
		TextOut(hmdc,20,20,"c:\\>",strlen("c:\\>")); //print tex
